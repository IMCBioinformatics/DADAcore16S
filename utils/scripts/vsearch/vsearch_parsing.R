...
if (snakemake@config[['FieldDB_after_GTDB']]==TRUE && file.exists(snakemake@input[["FieldDB_df"]]) && length(readLines(snakemake@input[["FieldDB_df"]])) > 0){

output_table_FieldDB <- read.table(file= snakemake@input[["FieldDB_df"]], sep = '\t', header = F)

# Create a vector of column names for the output table
names <- c("asv_seq", "taxonomy", "identity")

colnames(output_table_FieldDB) <- names


# Join the output_table_GTDB data frame with the output table for the FieldDB vsearch
output_table <- output_table_GTDB %>% left_join(output_table_FieldDB, by = "asv_seq", suffix = c("_GTDB", "_FieldDB"),multiple = "all") %>% 
    # Add a new 'Database' column that identifies which database each hit came from, based on the 'identity' columns
    mutate(Database = case_when(identity_GTDB > 0 ~ 'GTDB', identity_FieldDB > 0 ~ 'FieldDB'), .after = asv_len) %>%
    # Add a new 'identity' column that combines the 'ID' columns from both databases, preferring the GTDB ID if both are present
    mutate(identity = coalesce(identity_GTDB, identity_FieldDB), .after = Database) %>%
    # Add a new 'Taxa' column that combines the 'Taxa' columns from both databases, preferring the GTDB Taxa if both are present
    mutate(taxonomy = coalesce(taxonomy_GTDB, taxonomy_FieldDB), .after = identity) %>%
...
#Taking care of the fact that FieldDB acession numbers exist in different taxonomy columns 

if (snakemake@config[['FieldDB_after_GTDB']] == TRUE){
for(i in 1:nrow(unique_asvs)){
  if(is.na(unique_asvs$Num_hits[i])==FALSE & unique_asvs$Species_level[i]==FALSE & unique_asvs$Database[i]=="FieldDB"){
...
write.table(final_result_unique_id,snakemake@output[["parsed_collapsed_GTDB_FieldDB"]],row.names=F,sep="\t")


############ merging vsearch and dada2 results #############

# conditional to check the use of FieldDB database
...
if (snakemake@config[['FieldDB_after_GTDB']]==TRUE) {
  ...
  dd2 <- read.table(file = snakemake@input[["annotation"]], sep = ',', header = T) %>%
    select(-contains("_rdp"),-contains("_silva"),-contains("_FieldDB") ) 
}
...
# create a dataframe to colapse the taxonomy from vsearch + dada2_gtdb + dada2_FieldDB
colapsed <- df[1:3] 
...
# dada2 FieldDB assignments , if TRUE in the pipelie
if(snakemake@config[['FieldDB_after_GTDB']]==TRUE){
  FieldDB_dd2 <- df[c("kingdom_FieldDB","phylum_FieldDB","class_FieldDB","order_FieldDB","family_FieldDB","genus_FieldDB","species_FieldDB")]
}


# This loop checks if there is a Vsearch assignment to each ASV, if not it checks dada2_gtdb for assignment to any level, if not it tries dada2_FieldDB when FieldDB=TRUE
...
for(i in 1:nrow(df)){
  ...
  } else if (df$identity_vsearch[i]==0 & snakemake@config[['FieldDB_after_GTDB']]==TRUE) {
    colapsed[i,4:10]<- FieldDB_dd2[i,]
    colapsed[i,"database_final"]<-"FieldDB"
    colapsed[i,"package"]<-"DADA2"
